  ...
  .action(
    async (options: { force?: boolean; guild?: string; remove?: string }) => {
      ...
      if (force) {
        ...
        const recruitGuildId = process.env.DISCORD_RECRUIT_GUILD_ID;
        if (!recruitGuildId) {
          throw new Error(
            "DISCORD_RECRUIT_GUILD_ID is required when --guild is omitted.",
          );
        }

        console.log(
          `🏠 Deploying commands to recruit guild: ${recruitGuildId}`,
        );
