...
export class RecruitHandler {
  handleRecruitStep(
    ...
    switch (nextStep.step) {
      case "create-channel":
        void this.recruitStepCreateChannel(
          interaction as ButtonInteraction,
          nextStep as NextStepCreateChannel,
        );
        break;
      case "start-recruit":
        void this.recruitStepStartRecruit(
          interaction as ButtonInteraction,
          nextStep as NextStepStartRecruit,
        );
        break;
      case "start-enter-main-char":
        void this.recruitStepStartEnterMainChar(
          interaction as Message,
          nextStep as NextStepStartEnterMainChar,
        );
        break;
      case "enter-main-char":
        void this.recruitStepEnterMainChar(
          interaction as ButtonInteraction,
          nextStep as NextStepEnterMainChar,
        );
        break;
      case "seleted-recruit-route":
        void this.recruitStepSelectedRecruitRoute(
          interaction as StringSelectMenuInteraction,
          nextStep as NextStepSelectedRecruitRoute,
        );
        break;
      case "start-agree-infomal":
      case "start-agree-pvp":
        void this.recruitStepStartAgree(
          interaction as ButtonInteraction,
          nextStep as NextStepStartAgree,
        );
        break;
      case "start-seat-registration":
        void this.recruitStepStartSeatRegistration(
          interaction as ButtonInteraction,
          nextStep as NextStepStartSeatRegistration,
        );
        break;
      case "confirm-seat-registration":
        void this.recruitStepConfirmSeatRegistration(
          interaction as ButtonInteraction,
          nextStep as NextStepConfirmSeatRegistration,
        );
        break;
      case "finish-recruit":
        void this.recruitStepFinishRecruit(
          interaction as ButtonInteraction,
          nextStep as NextStepFinishRecruit,
        );
        break;
    }
  }
  ...
  isBanTarget(banCharInfo: BanCharInfo) {
    ...
    const reason =
      `주의! 현재 리크룻 중인 캐릭터 **${banCharInfo.char[0]}**은(는) 밴 리스트에 등재되어 있습니다. 반드시 참고하시고 리크룻 바랍니다.\`\`\`해당 캐릭터가 포함된 밴 리스트는 다음과 같습니다:\n` +
      banReasonString +
      "```";
  ...
  async getMessageFromContents(channel: TextChannel, contents: string[]) {
    const returnObject: Record<string, Message> = {};
    ...
    while (messageIdPointer) {
      ...
      if (messageIdPointer === "temp") {
        messagePage = await channel.messages.fetch({ limit: 1 });
      } else {
        messagePage = await channel.messages.fetch({
          limit: 100,
          before: messageIdPointer,
        });
      }
      ...
      for (const msg of messagePage) {
        ...
        if (
          msg[1].author.id == "1091569359794737242" &&
          recruitStringArrIndex !== -1
        ) {
   ...
   **/
  async recruitStepCreateChannel(
    interaction: ButtonInteraction,
    recruitData: NextStepCreateChannel,
  ) {
    const matchingChannels = await interaction.guild?.channels
      .fetch()
      .then((channels) =>
        channels.filter(
          (c) =>
            ...
            c.name.split("-")[0] === interaction.user.tag,
        ),
      );
    ...
    if (!consultingCategory || !recruiterRole)
      throw Error("리크루터 롤 혹은 상담중 카테고리가 존재하지 않습니다.");

    const consultingChannel = await (
      consultingCategory as CategoryChannel
    ).children.create({
      name:
        interaction.user.tag +
        "-" +
        recruitStepData[recruitData.step][recruitData.type].type,
    ...
    const response = {
      content:
        `**${interaction.user.tag}` +
        recruitStepData[recruitData.step][recruitData.type].response,
   ...
   **/
  async recruitStepStartRecruit(
    interaction: ButtonInteraction,
    recruitData: NextStepStartRecruit,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      interaction.channel as TextChannel,
      recruitStepData[recruitData.step][recruitData.type].response,
    );
    if (passedThisStep) {
      await interaction.reply(
        "이미 꼽을 선택하셨습니다. 다음 절차를 진행해 주세요. \n혹시 잘못 선택하셨다면 리크루터에게 문의해 주세요.",
      );
      return;
    }

    await (interaction.channel as TextChannel).setName(
      (interaction.channel as TextChannel).name + `-${recruitData.type}`,
    );
    await interaction.reply(
      recruitStepData[recruitData.step][recruitData.type].response,
    );
   ...
   **/
  async recruitStepStartEnterMainChar(
    message: Message,
    recruitData: NextStepStartEnterMainChar,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      message.channel as TextChannel,
      recruitStepData[recruitData.step].content,
    );
    if (passedThisStep) {
      if (message.channel instanceof TextChannel) {
        await message.channel.send(
          "이미 조건을 숙지하셨습니다. 다음 절차를 진행해 주세요.",
        );
      }
      return;
    }
   ...
   **/
  async recruitStepEnterMainChar(
    interaction: ButtonInteraction,
    recruitData: NextStepEnterMainChar,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      interaction.channel as TextChannel,
      recruitStepData[recruitData.step].response,
    );
    if (passedThisStep) {
      await interaction.reply(
        "이미 메인 캐릭터를 확인했습니다. 다음 절차를 진행해 주세요. \n혹시 잘못 입력하셨다면 리크루터에게 문의해 주세요.",
      );
      return;
    }
    ...
    if (submitted) {
      ...
      const mainCharacterName = submitted.fields
        .getTextInputValue("mainCharNameInput")
        .trim();
      // TODO: 이름에 대한 유효성 검사 추가

      const esiRequest = new EsiRequester();
      const mainCharacterIds = await esiRequest.getIdsFromNames([
        mainCharacterName,
      ]);
      ...
      const mainCharacterCorpHistory =
        await esiRequest.getCorpHistoryFromCharId(mainCharacterId);
      const corpNames = await esiRequest.getNamesFromIds([
        ...new Set(
      ...
      const recruiterRole = submitted.guild?.roles.cache.find(
        (role) => role.name == "리크루터",
      );
      const { isBaned, reason } = this.isBanTarget(charInfoByType);
      const workingChannel = submitted.guild?.channels.cache.find(
        (channel) => channel.name === "리크루터-working",
      ) as TextChannel;
   ...
   **/
  async recruitStepSelectedRecruitRoute(
    interaction: StringSelectMenuInteraction,
    recruitData: NextStepSelectedRecruitRoute,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      interaction.channel as TextChannel,
      recruitStepData[recruitData.step].response,
    ...
    if (passedThisStep) {
      await interaction.reply(
        "이미 리크룻 경로를 선택하셨습니다. 다음 절차를 진행해 주세요. \n혹시 잘못 선택하셨다면 리크루터에게 문의해 주세요.",
      );
      return;
    }
    ...
    const recruitRouteEmbed = new EmbedBuilder().setDescription(
      "```" + "경로: " + selectedRecruitRoute + result + "```",
    );
   ...
   **/
  async recruitStepStartAgree(
    interaction: ButtonInteraction,
    recruitData: NextStepStartAgree,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      interaction.channel as TextChannel,
      recruitStepData[recruitData.step].response,
    ...
    const type = (interaction.channel as TextChannel).name.split("-")[2] as
      | "nisuwaz"
      | "consui";
   ...
   **/
  async recruitStepStartSeatRegistration(
    interaction: ButtonInteraction,
    recruitData: NextStepStartSeatRegistration,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      interaction.channel as TextChannel,
      recruitStepData[recruitData.step].response,
    ...
    if (passedThisStep) {
      await interaction.reply(
        "이미 모든 동의를 완료하셨습니다. 다음 절차를 진행하여 주세요.",
      );
      return;
    }
   ...
   **/
  async recruitStepConfirmSeatRegistration(
    interaction: ButtonInteraction,
    recruitData: NextStepConfirmSeatRegistration,
  ) {
    const passedThisStep = await this.isRecruitStepExist(
      interaction.channel as TextChannel,
      recruitStepData[recruitData.step].response,
    ...
    const recruitSummary = (await this.getMessageFromContents(
      interaction.channel as TextChannel,
      [
        recruitStepData["seleted-recruit-route"].response,
        recruitStepData["enter-main-char"].response,
      ],
    )) as Record<string, Message>;
    const [maincharName, maincharId] = recruitSummary[
      recruitStepData["enter-main-char"].response
    ].embeds[0].data.title
      ?.split(":")
    ...
    if (!maincharName || !maincharId)
      throw Error("캐릭터 정보를 불러올 수 없습니다.");
    ...
    try {
      ...
      await interaction.editReply(
        "시트 가입 인증에 실패했습니다. 10 ~ 30분 후 다시 시도해 주세요. \n만약 계속 실패한다면 리크루터에게 문의해 주세요.",
      );
    }

    const recruitRouteString =
      recruitSummary[
        recruitStepData["seleted-recruit-route"].response
   ...
   * recruitStepStartSeatRegistration 다음 단계. 동의 버튼 클릭을 받고 시트 가입을 확인한 후 리크룻을 마칩니다.
   **/
  async recruitStepFinishRecruit(
    interaction: ButtonInteraction,
    recruitData: NextStepFinishRecruit,
  ) {
    if (recruitData.type === "no") {
      // 취소
      await interaction.update({
        content: "상담 완료 명령이 취소되었습니다.",
        components: [],
      });
      return;
    }
    ...
    const targetCategories = interaction.guild?.channels.cache.filter(
      (c) => c.type === ChannelType.GuildCategory && c.name === "상담완료",
    );

    const channel = interaction.channel as TextChannel;
    if (targetCategories?.size !== 1) {
      await interaction.editReply(
        "상담완료 카테고리가 존재하지 않거나 너무 많습니다.",
      );
      return;
    }

    await channel.setParent(
      (targetCategories.entries().next().value as [string, CategoryChannel])[1],
    );
    ...
    await channel.send(
      ...
      `${interaction.member?.toString()}님이 상담완료 명령을 사용하여 이 채널을 완료 처리 하였습니다.`,
    );
  }
}
