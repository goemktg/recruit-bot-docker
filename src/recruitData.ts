import { ButtonBuilder, ButtonStyle, ActionRowBuilder, ModalBuilder, TextInputBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, TextInputStyle } from 'discord.js';
...
export const recruitStepData = {
	...
	'start-enter-main-char': {
		content: '조건 숙지를 확인했습니다. 다음으로 하단 버튼을 눌러 나오는 메뉴에서 자신의 **메인 캐릭터 명**을 입력해 주세요.',
		row: [
	...
	'enter-main-char': {
		response: '메인 계정이 확인되었습니다. 아래 정보가 맞는지 확인하시고, 그 아래 박스에서 리크룻 경로를 선택해 주세요.',
	...
	'start-agree-infomal': {
		response: '저희 코퍼레이션은 기본적으로 반말을 허용하고 있으나, 원하시면 존대도 사용 가능합니다. 동의하시면 아래 버튼을 눌러주세요.',
		responseAgain: '이미 경로를 확인하셨습니다. 다음 절차를 진행해 주세요.\n혹시 잘못 선택하셨다면 리크루터에게 문의해 주세요.',
		'nisuwaz': {
			row: new ActionRowBuilder<ButtonBuilder>()
				.addComponents(new ButtonBuilder()
